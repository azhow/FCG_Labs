...
//Camera position, view and up vectors
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        g_deltaTime = curFrame - g_lastFrame;
        ...
        glm::mat4 view = glm::lookAt(g_cp, g_cp+g_vvv, g_up);
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mod)
{
    ...
    if(glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) g_cp += cameraSpeed * g_vvv;
    if(glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) g_cp -= cameraSpeed * g_vvv;
    if(glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) g_cp -= glm::normalize(glm::cross(g_vvv, g_up)) * cameraSpeed;
    if(glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) g_cp += glm::normalize(glm::cross(g_vvv, g_up)) * cameraSpeed;
}
